...
const GEMS = [
  { name: 'Coral',  colour: 0xff5e78, css: '#ff5e78', scheme: 'doubles every match' },
  { name: 'Amber',  colour: 0xffc93c, css: '#ffc93c', scheme: 'steady 50, never moves' },
  { name: 'Lagoon', colour: 0x2fe0d0, css: '#2fe0d0', scheme: '+6 every match' },
  { name: 'Orchid', colour: 0xb98bff, css: '#b98bff', scheme: 'rarer on board = richer' },
  { name: 'Palm',   colour: 0x7ee060, css: '#7ee060', scheme: 'steady 15' },
  { name: 'Mango',  colour: 0xff9f45, css: '#ff9f45', scheme: 'grows every stage' },
];
...
const STAGES = [
  { name: 'Sunrise Shallows', sky: ['#ffe7bd', '#9beee6', '#3ecfc4'], fog: '#9beee6', sand: '#ffe9b8', sun: 0xfff1cf, hemi: 0xcffcf5, featured: 2, mult: 3 },
  { name: 'Coral Garden',     sky: ['#ffd9e5', '#9df0dc', '#3bd4bd'], fog: '#a9f0df', sand: '#ffdfc2', sun: 0xffe9dd, hemi: 0xffe3ec, featured: 0, mult: 3 },
  { name: 'Golden Hour',      sky: ['#ffd27a', '#ffe9ac', '#5fd9c4'], fog: '#ffe2a4', sand: '#ffd9a0', sun: 0xffdf9e, hemi: 0xfff0c8, featured: 5, mult: 3 },
  { name: 'Orchid Cove',      sky: ['#f3d9ff', '#b7ecf2', '#59d4d0'], fog: '#c8ecf4', sand: '#f6e3ff', sun: 0xfbeaff, hemi: 0xe9dcff, featured: 3, mult: 4 },
  { name: 'Pearl Morning',    sky: ['#ffffff', '#d9f7f2', '#7fe0d6'], fog: '#e2f8f4', sand: '#fff4d8', sun: 0xffffff, hemi: 0xf3fffd, featured: 1, mult: 3 },
  { name: 'Palm Reef',        sky: ['#f0ffc8', '#a9f0c9', '#41d3b0'], fog: '#bdf2d2', sand: '#f2eeb4', sun: 0xfaffdc, hemi: 0xdcffe4, featured: 4, mult: 4 },
];
...
scene.fog = new THREE.Fog(0x9beee6, 30, 95);
...
// -- lights
const hemi = new THREE.HemisphereLight(0xcffcf5, 0xf7e2ae, 0.95);
scene.add(hemi);
...
const swayers = [];
{
  ...
  for (let i = 0; i < 11; i++) {
    ...
    for (let k = 0; k < n; k++) {
      ...
      let m;
      if (kind < 0.45) {
        m = new THREE.Mesh(new THREE.ConeGeometry(0.28 + Math.random() * 0.3, 1.4 + Math.random() * 2.2, 7), mat);
        m.position.y = m.geometry.parameters.height / 2;
      } else if (kind < 0.8) {
        m = new THREE.Mesh(new THREE.SphereGeometry(0.5 + Math.random() * 0.55, 10, 8), mat);
        m.position.y = 0.4;
      } else {
...
const fishes = [];
{
  ...
  for (let i = 0; i < 6; i++) {
    ...
    const body = new THREE.Mesh(new THREE.ConeGeometry(0.22, 0.75, 8), mat);
    body.rotation.z = -Math.PI / 2; // point along +x
    const tail = new THREE.Mesh(new THREE.ConeGeometry(0.18, 0.34, 6), mat);
    tail.rotation.z = Math.PI / 2;
    tail.position.x = -0.5;
    g.add(body, tail);
    g.userData = {
      radius: 13 + Math.random() * 12,
...
function dropMesh(mesh, fromRow, toRow, c) {
  ...
  const dur = 105 + 130 * Math.sqrt(cellsFallen);
  return tween(dur, (p) => {
